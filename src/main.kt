    ...
    fun main() {
        ...
        fun parseValues(): Arguments {
            ...
            return when (arg.count()) {
                2 -> {
                    Arguments(login = arg[0], pass = arg[1])
                }
                4 -> {
                    Arguments(login = arg[0], pass = arg[1], role = stringToRole(arg[2]), res = arg[3])
                }
                7 -> {
                    Arguments(login = arg[0], pass = arg[1], role = stringToRole(arg[2]), res = arg[3], ds = arg[4], de = arg[5], vol = arg[6])
                }
                else -> {
                    printHelp()
                    exitProcess(1)
        ...
        fun countArguments(): Int {
            var count = 0
            if (argument.login !== null) {
                count++
            }
            if (argument.pass !== null) {
                count++
            }
            if (argument.role !== null) {
                count++
            }
            if (argument.res !== null) {
                count++
            }
            if (argument.de !== null) {
                count++
        ...
        fun isLoginValid(): Boolean {
            ...
            val checkReg = argument.login?.let { regex.matches(it) }
            if (checkReg == true) {return true}
                else {exitProcess(2)}
        }

        fun logBase(): String? {
            return dataBaseUsers.firstOrNull { it.login == argument.login }?.login
        }

        fun hasLogin(): Boolean {
            if (logBase() !== argument.login) {
                return true
            } else {
                exitProcess(3)
            }
        ...
        fun isPassValid(): Boolean {
            val salt = dataBaseUsers.first { it.login == logBase() }.salt
            val hashInputPassword = DigestUtils.md5Hex(argument.pass) + salt
            val basePassword = dataBaseUsers.first { it.login == logBase() }.hash + salt
            if (hashInputPassword == basePassword) {
                return true
            } else exitProcess(4) }
        ...
        fun isVolumeValid(): Boolean {
            ...
            val checkReg = argument.vol?.let { regex.matches(it) }
            if (checkReg == true) {return true}
            else {exitProcess(7)}
        }
        ...
        fun hasAuthentication(count: Int) {
            when (count) {
                ...
                4 -> {
                    if (isLoginValid() && hasLogin() && isPassValid() && isResValid()) {
                        exitProcess(0)
                    }
                }
                7 -> {
                    if (isLoginValid() && hasLogin() && isPassValid() && isResValid() && isVolumeValid() && isDateValid()) {
                        exitProcess(0)
                    }
                }
                else -> {
        ...
        hasAuthentication(countArguments())
    }
